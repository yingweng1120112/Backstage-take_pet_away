...
//有拿到資料
$r = $pdo->query("SELECT * FROM psycological_test WHERE question_id=$questionId")->fetch();
if (empty($r)) {
  header('Location: hearttest_question.php');
  exit;
}
        ...
        <div class="card-body container ">
          <form name="form1" class="container row" onsubmit="sendData(event)">
            ...
            <div class="row mb-3">
              ...
              <div class="col-md-12">
                <textarea class="form-control " id="question" name="question_content" cols="30" rows="3" placeholder="最多輸入 25 個字" maxlength="25"><?= $r['question_content'] ?></textarea>
...
<script>
  ...
  function sendData(e) {
    ...
    if (isPass) {
      ...
      fetch('hearttest_question_edit-api.php', {
          method: 'POST',
          body: fd
        })
        .then(r => r.json())
